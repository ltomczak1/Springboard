...
SELECT name, 
	CASE WHEN monthlymaintenance <= 100 THEN 'Cheap'
    	ELSE 'Expensive' END AS maintenance_costs
FROM Facilities
ORDER BY maintenance_costs
...
GROUP BY f.name
HAVING SUM(CASE WHEN memid = 0 THEN slots * f.guestcost
	ELSE slots * membercost
	END) < 1000
ORDER BY revenue;
...
FROM Bookings b 
    INNER JOIN Facilities f USING (facid)
HAVING b.memid != 0
ORDER BY b.memid, m.name

/* Q13: Find the facilities usage by month, but not guests */
SELECT strftime('%m', starttime) AS Month, f.name AS facility_name, COUNT(*) AS monthly_usage
FROM Bookings b
    INNER JOIN Facilities f USING (facid)
WHERE b.memid != 0
GROUP BY Month, name
...
*The last two were hard. Can you provide the solution for these?